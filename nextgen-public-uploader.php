<?php
/*
Plugin Name: FS NextGEN Public Uploader
...
Version: 1.9
Author: WebDevStudios + Jewel Mlnarik
...
function npu_translate() {
	load_plugin_textdomain( 'nextgen-public-uploader', false, dirname( plugin_basename( __FILE__ ) ) . '/nextgen-public-uploader/languages/' );
}
...
function npu_plugin_activation() {
	...
	if ( ! get_option( 'npu_default_gallery' ) ) {
		...
		update_option( 'npu_notlogged', 				__( 'Please log into your photographer account to upload session photos.', 'nextgen-public-uploader' ) );
		update_option( 'npu_upload_success', 			__( 'Your images have been successfully uploaded.', 'nextgen-public-uploader' ) );
		update_option( 'npu_upload_failed', 			__( 'Bummer. Your upload failed. Please try again or ask for help in Slack.', 'nextgen-public-uploader' ) );
...
function load_nggpu() {
	// Upload Form Path
	require_once( dirname( __FILE__ ) . '/nextgen-public-uploader/inc/npu-upload.php' );
	include_once( dirname( __FILE__ ) . '/nextgen-public-uploader/tinymce/tinymce.php' ); // TinyMCE
}
add_action( 'plugins_loaded', 'load_nggpu' );

define( 'nextgenPublicUpload_URLPATH', WP_PLUGIN_URL . '/' . plugin_basename( dirname(__FILE__) ) . '/nextgen-public-uploader/' );
...
add_action( 'admin_menu', 'npu_plugin_menu', 11 );
