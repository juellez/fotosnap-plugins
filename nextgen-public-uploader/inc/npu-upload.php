...
if ( ! class_exists( 'npuGalleryUpload' ) ) {
	...
	class npuGalleryUpload {
		...
		public function display_image_upload_input( $gal_id = 0, $context = 'shortcode', $disable = false, $name = 'galleryselect' ) {
			...
			$strOutput .= "\n\t<div class=\"uploader\">";
			$strOutput .= "\n\t<input type=\"file\" multiple name=\"imagefiles[]\" id=\"imagefiles\"{$disabled} />";
			$strOutput .= "\n</div>";
		...
		public function sendEmail() {
			...
			if ( get_option( 'npu_notification_email' ) ) {
				...
				$subject = apply_filters( 'npu_gallery_upload_send_email_subject', __( 'New Client Photos Pending Review', 'nextgen-public-uploader' ), $this );
				$headers = array("From: {$email}");

				$message = "A new batch of images has been submitted by {$user}. ";
				$message .= "Please review and move images to the appropriate client gallery and session by going to the default <a href=\"https://fotosnap.co/wp-admin/admin.php?page=nggallery-manage-gallery&mode=edit&gid=6\">'Newly Uploaded' Gallery</a>. \n\n";
				$message .= "Once there, you can select the images to move to a new or existing client gallery (linked to a client session) by selecting the Bulk Action > Move option. \n\n";
				// https://fotosnap.co/wp-admin/admin.php?page=nggallery-manage-gallery&mode=edit&gid=6&paged=1
				// https://fotosnap.co/wp-admin/admin.php?page=nggallery-manage-gallery
