...
class UploaderNggAdmin extends nggAdmin {
	...
	function upload_images() {
		...
		foreach( $_FILES["imagefiles"]["name"] as $key => $fname ) {
			if ( $_FILES["imagefiles"]["error"][$key] == 0 ) {
				$entropy = '';
				$temp_file = $_FILES["imagefiles"]['tmp_name'][$key];
				$filepart = pathinfo( strtolower( $fname ) );
				...
				if ( !in_array( $filepart['extension'], $ext ) || !@getimagesize( $temp_file ) ) {
					self::show_error( '<strong>' . $fname . '</strong>' . __( 'is not a valid file.', 'nextgen-public-uploader' ) );
					continue;
				}
				...
				// Save Temporary File
				if ( !@move_uploaded_file( $_FILES["imagefiles"]['tmp_name'][$key], $dest_file ) ) {
		...
		if ( count( $imageslist ) > 0 ) {
			...
			self::show_error( __( 'Your photos have been uploaded. Great work!', 'nextgen-public-uploader' ) );
		}
		return;
	} // End Function
